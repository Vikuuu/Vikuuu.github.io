...
programming language, so I will have to convert it from ruby to go, but ruby being ruby seeing it just currently seems to
me like pseudo-code (almost like magic), that's good for me I will have to take the psedo-code and the concepts mentioned
in the books and convert it into the go code, for now I won't be going to optimised it or any thing like that, I will
...
Lets say we have a file hello.txt, and it contains only a word "hello" in it. When we see the content of the gits object
...
> output, meaning if the content of the two inputs are same then the output of those will be same, this can tell us if the
